        ...
        try:
            async with engine.begin() as conn:
                ...
                if dry_run:
                    ...
                    print(f"  4. NULL discord_event_id for meeting_ids: {SHIFTED_MEETING_IDS}")
                    print("\n[DRY RUN] No changes made.")
                    return

                # Step 1: Update cohort number_of_group_meetings
                cohort_result = await conn.execute(text("""
                ...
                for meeting_id, new_date in MEETING_SHIFTS.items():
                    ...
                    row = result.fetchone()
                    print(f"--- Step 2: Shifted meeting_id={row[0]} (group {row[2]}, meeting {row[1]}) -> {row[3]}")
                ...
                for group_id, scheduled_at in MEETING_7.items():
                    ...
                    row = result.fetchone()
                    print(f"--- Step 3: Inserted meeting_id={row[0]} (group {row[1]}, meeting {row[2]}) -> {row[3]}")
