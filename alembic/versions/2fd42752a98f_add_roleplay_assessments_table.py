...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "score_data", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_roleplay_assessments_session_id_chat_sessions"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("assessment_id", name=op.f("pk_roleplay_assessments")),
    )
    op.create_index(
        "idx_roleplay_assessments_session_id",
        "roleplay_assessments",
        ["session_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        "idx_roleplay_assessments_session_id", table_name="roleplay_assessments"
    )
