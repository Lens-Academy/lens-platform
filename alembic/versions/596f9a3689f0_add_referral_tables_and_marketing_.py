...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.user_id"],
            name=op.f("fk_referral_links_user_id_users"),
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        "idx_referral_links_user_id", "referral_links", ["user_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["link_id"],
            ["referral_links.link_id"],
            name=op.f("fk_referral_clicks_link_id_referral_links"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("click_id", name=op.f("pk_referral_clicks")),
    )
    op.create_index(
        "idx_referral_clicks_link_id", "referral_clicks", ["link_id"], unique=False
    )

    # Add marketing consent and referral columns to users
    op.add_column(
        "users", sa.Column("cookies_marketing_consent", sa.Text(), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column(
            "cookies_marketing_consent_at",
            postgresql.TIMESTAMP(timezone=True),
            nullable=True,
        ),
    )
    ...
    op.create_check_constraint(
        "ck_users_valid_cookies_marketing_consent",
        "users",
        "cookies_marketing_consent IN ('accepted', 'declined')",
    )
    op.create_foreign_key(
        op.f("fk_users_referred_by_link_id_referral_links"),
        "users",
        "referral_links",
        ["referred_by_link_id"],
        ["link_id"],
...
def downgrade() -> None:
    op.drop_constraint(
        op.f("fk_users_referred_by_link_id_referral_links"), "users", type_="foreignkey"
    )
    op.drop_constraint(
        "ck_users_valid_cookies_marketing_consent", "users", type_="check"
    )
