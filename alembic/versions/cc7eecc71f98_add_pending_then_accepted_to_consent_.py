...
def upgrade() -> None:
    op.execute(
        ...
        "DROP CONSTRAINT ck_referral_clicks_consent_state_values"
    )
    op.execute(
        "ALTER TABLE referral_clicks "
        "ADD CONSTRAINT ck_referral_clicks_consent_state_values "
...
def downgrade() -> None:
    op.execute(
        "UPDATE referral_clicks SET consent_state = 'accepted' "
        "WHERE consent_state = 'pending_then_accepted'"
    )
    op.execute(
        "ALTER TABLE referral_clicks "
        "DROP CONSTRAINT ck_referral_clicks_consent_state_values"
