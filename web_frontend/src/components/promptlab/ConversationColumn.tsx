    ...
    <div className="flex flex-col h-full w-[280px] min-w-[280px] border-r border-gray-200 last:border-r-0">
      ...
      {slot.error && (
        <div className="px-3 py-1.5 bg-red-50 text-[10px] text-red-600 flex items-center gap-1">
          <span className="truncate">
            {slot.error.length > 80
              ? "Request failed. Check console."
              : slot.error}
          </span>
          <button
            onClick={slot.dismissError}
      ...
      >
        {slot.messages.map((msg, index) => {
          ...
          const isDimmed =
            slot.selectedMessageIndex !== null &&
            index > slot.selectedMessageIndex;
            ...
            >
              ...
              {msg.isRegenerated && msg.originalContent && (
                <div className="mb-1">
                  ...
                  >
                    <svg
                      className={`w-2.5 h-2.5 transition-transform ${expandedOriginals.has(index) ? "rotate-90" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              {msg.isRegenerated && msg.thinkingContent && (
                <div className="mb-1">
                  ...
                  >
                    <svg
                      className={`w-2.5 h-2.5 transition-transform ${expandedThinking.has(index) ? "rotate-90" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              <div
                onClick={() => {
                  if (isAssistant && !slot.isStreaming)
                    slot.selectMessage(index);
                }}
              ...
              >
                ...
                <div
                  className={
                    isAssistant ? "prose-compact" : "whitespace-pre-wrap"
                  }
                >
                  {isAssistant ? (
      ...
      <form
        ...
        className="flex gap-1 p-2 border-t border-gray-100 items-end"
      >
        <textarea
          ref={textareaRef}
          value={followUpInput}
        ...
        <button
          type="submit"
          disabled={
            !canSendFollowUp || slot.isStreaming || !followUpInput.trim()
          }
