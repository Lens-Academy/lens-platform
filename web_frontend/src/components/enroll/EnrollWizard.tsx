...
export default function EnrollWizard({
  ...
  const handleSubmit = async () => {
    ...
    // Pre-validate: block availability submission if cohort doesn't accept it
    if (
      !formData.selectedGroupId &&
      selectedCohort &&
      selectedCohort.accepts_availability_signups === false
    ) {
      alert(
        "This cohort is not accepting availability submissions. Please select a group directly or choose a different cohort.",
      );
      return;
    }
      ...
      {currentStep === 3 &&
        ...
        (!forceAvailabilityMode || !selectedCohortAcceptsAvailability) ? (
          <GroupSelectionStep
            ...
            showAvailabilityOption={selectedCohortAcceptsAvailability}
