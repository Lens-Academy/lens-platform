...
<task type="auto">
  ...
  <action>
    ...
    **4. Implement enforceVoice support:**
    ...
    - Consider adding `answer_metadata: { voice_used: true }` when the answer includes transcribed voice content. Track this in a ref and pass to the auto-save hook's metadata.

    **5. Track voice usage in answer_metadata:**

    When transcription completes, set a flag indicating voice was used. When the auto-save fires, include `{ voice_used: true }` in the `answer_metadata` (via the useAutoSave hook — may need to add a `setMetadata` function or accept metadata as a parameter).

    The simplest approach: add an optional `answerMetadata` parameter to the useAutoSave hook's save function, or maintain a `metadataRef` in AnswerBox that gets passed through. Since useAutoSave already has access to the API calls, the cleanest approach is to have AnswerBox pass metadata updates alongside text updates.
