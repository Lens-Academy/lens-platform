# Requirements Archive: v3.1 AI Roleplay
...
- [x] **CONT-01**: Course creators can define `#### Roleplay` segments in content markdown with `id::`, `content::`, `ai-instructions::` fields
- [x] **CONT-02**: Roleplay segments support optional `assessment-instructions::` field for AI scoring
- [x] **CONT-03**: Roleplay segments support optional `opening-message::` field for AI greeting
- [x] **CONT-04**: Roleplay segments work in all section types (page, lens-article, lens-video)

### Voice & TTS

- [x] **VOICE-01**: AI roleplay responses are converted to speech via Inworld TTS
- [x] **VOICE-02**: Backend streams LLM text tokens to Inworld via WebSocket, receives audio chunks back
- [x] **VOICE-03**: Backend streams audio chunks to the browser for playback while Inworld is still generating
- [x] **VOICE-04**: Voice-only mode: student uses mic input, AI responds with audio (no keyboard)
- [x] **VOICE-05**: Text mode: student uses keyboard, AI responds with text only (no TTS)

### Conversation

- [x] **CONV-01**: Student has multi-turn streaming conversation with AI character
- [x] **CONV-02**: Character name displayed distinctly from tutor chat
- [x] **CONV-03**: Student can use voice input via existing mic/speech-to-text
- [x] **CONV-04**: Conversation persists across page refresh
- [x] **CONV-05**: Student clicks a completion button to end the conversation
- [x] **CONV-06**: After completion, conversation shows done state with input disabled
- [x] **CONV-07**: Scenario briefing card displayed above conversation before first message
- [x] **CONV-08**: AI character sends opening message to start the conversation
- [x] **CONV-09**: Student can retry roleplay with a fresh conversation ("Try again")

### Assessment

- [x] **ASMNT-01**: AI scores full conversation transcript against author-defined rubric after completion (score card UI deferred; feedback chat provides assessment context)
- [x] **ASMNT-02**: Roleplay segments work inside test sections (like question segments)
- [x] **ASMNT-03**: Post-conversation feedback chat available after assessment (server-side context injection)

### Infrastructure

- [x] **INFRA-01**: Roleplay sessions isolated via `roleplay_id` column on `chat_sessions` with partial unique indexes
- [x] **INFRA-02**: Roleplay prompt assembly in `core/modules/roleplay.py`, separate from tutor `chat.py`
...
| VOICE-01 | Phase 9 | Complete |
| VOICE-02 | Phase 9 | Complete |
| VOICE-03 | Phase 9 | Complete |
...
| CONV-05 | Phase 10 | Complete |
| CONV-06 | Phase 10 | Complete |
| CONV-07 | Phase 10 | Complete |
| CONV-08 | Phase 10 | Complete |
| CONV-09 | Phase 10 | Complete |
| ASMNT-01 | Phase 11 | Complete |
| ASMNT-02 | Phase 11 | Complete |
| ASMNT-03 | Phase 11 | Complete |
