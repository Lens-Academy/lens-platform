...
from core.modules.course_loader import load_course, get_due_by_meeting, get_all_module_slugs
    ...
    if all_section_ids:
        time_query = (
            select(
                ...
                func.sum(user_content_progress.c.total_time_spent_s).label("total_time"),
            )
            .where(
                and_(
...
def render_engagement_dm(stats: list[dict]) -> str | None:
    ...
    for stat in stats:
        ...
        if "sections_completed" in stat:
            parts.append(f"\U0001f4d6 {stat['sections_completed']}/{stat['sections_total']} sections")
        ...
        async with get_connection() as conn:
            ...
            for slug in get_all_module_slugs(course_slug):
                entries = await gather_group_study_data(
                    conn, group_id=group_id, module_slug=slug, today=today,
                )
