...
@pytest.mark.asyncio
async def test_claim_progress_records_merges_with_existing(test_user_id, session_token):
    ...
    async with get_transaction() as conn:
        # The claim will try to update session_token records to set user_id
...
async def test_get_or_create_progress_prefers_user_id(
    test_user_id, session_token, content_id
):
    """When both user_id and session_token are provided, user_id takes precedence."""
    async with get_transaction() as conn:
        progress = await get_or_create_progress(
            conn,
            user_id=test_user_id,
            session_token=session_token,  # Should be ignored
            content_id=content_id,
            content_type="lens",
            content_title="Test",
        )
    ...
    assert progress["user_id"] == test_user_id
    # session_token should not be set since user_id takes precedence
    assert progress["session_token"] is None
...
async def test_update_time_spent_no_identity():
    """update_time_spent with neither user_id nor session_token should do nothing."""
    content_id = uuid.uuid4()
...
async def test_get_module_progress_no_identity():
    """get_module_progress with neither user_id nor session_token should return empty."""
    lens_ids = [uuid.uuid4() for _ in range(3)]
